...
pkgdatadir = $(datadir)/super8core
pkgincludedir = $(includedir)/super8core
pkglibdir = $(libdir)/super8core
pkglibexecdir = $(libexecdir)/super8core
...
CONFIG_HEADER = $(top_builddir)/src/config/super8-config.h
CONFIG_CLEAN_FILES = libsuper8consensus.pc share/setup.nsi \
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	$(srcdir)/libsuper8consensus.pc.in \
	...
	$(top_srcdir)/src/config/super8-config.h.in \
...
ACLOCAL = ${SHELL} /tmp/Super8/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /tmp/Super8/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /tmp/Super8/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /tmp/Super8/build-aux/missing automake-1.15
AWK = gawk
BDB_CFLAGS = 
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = super8-cli
BITCOIN_DAEMON_NAME = super8d
BITCOIN_GUI_NAME = super8-qt
BITCOIN_TX_NAME = super8-tx
...
COPYRIGHT_HOLDERS_FINAL = The Super8 Core developers
COPYRIGHT_HOLDERS_SUBSTITUTION = Super8 Core
COPYRIGHT_YEAR = 2019
CPP = gcc -E
...
MAINT = 
MAKEINFO = ${SHELL} /tmp/Super8/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = super8core
PACKAGE_BUGREPORT = https://github.com/super8pay/super8/issues
PACKAGE_NAME = Super8 Core
PACKAGE_STRING = Super8 Core 1.0.0
PACKAGE_TARNAME = super8core
PACKAGE_URL = https://super8.org/
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /tmp/Super8
abs_srcdir = /tmp/Super8
abs_top_builddir = /tmp/Super8
abs_top_srcdir = /tmp/Super8
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /tmp/Super8/build-aux/install-sh
...
pkgconfig_DATA = libsuper8consensus.pc
...
DIST_CONTRIB = $(top_srcdir)/contrib/super8-cli.bash-completion \
	       $(top_srcdir)/contrib/super8-tx.bash-completion \
	       $(top_srcdir)/contrib/super8d.bash-completion \
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info \
  leveldb_baseline.info test_super8_filtered.info total_coverage.info \
  baseline_filtered.info rpc_test.info rpc_test_filtered.info \
  leveldb_baseline_filtered.info test_super8_coverage.info test_super8.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/Super8-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/super8-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/super8-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/super8-config.h
$(top_srcdir)/src/config/super8-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/super8-config.h src/config/stamp-h1
libsuper8consensus.pc: $(top_builddir)/config.status $(srcdir)/libsuper8consensus.pc.in
...
$(OSX_APP)/Contents/MacOS/Super8-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Super8-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Super8-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_super8.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_super8 -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_super8_filtered.info: test_super8.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#rpc_test.info: test_super8_filtered.info
...
#test_super8_coverage.info: baseline_filtered_combined.info test_super8_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_super8_filtered.info -o $@

#total_coverage.info: baseline_filtered_combined.info test_super8_filtered.info rpc_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_super8_filtered.info -a rpc_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_super8.coverage/.dirstamp:  test_super8_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_super8.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf coverage_percent.txt test_super8.coverage/ total.coverage/ qa/tmp/ cache/ $(OSX_APP)

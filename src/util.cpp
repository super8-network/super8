...
const char * const BITCOIN_CONF_FILENAME = "super8.conf";
const char * const BITCOIN_PID_FILENAME = "super8d.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "super8" is a composite category enabling all Super8-related debug output
                if(ptrCategory->count(std::string("super8"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Super8
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Super8
    // Mac: ~/Library/Application Support/Super8
    // Unix: ~/.super8
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Super8";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Super8";
#else
    // Unix
    return pathRet / ".super8";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty super8.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override super8.conf

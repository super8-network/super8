...
*Setup instructions for a Gitian build of Super8 Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Super8
...
- [Building Super8 Core](#building-super8-core)
...
Clone the git repositories for Super8 Core and Gitian.
...
in the Super8 Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Super8 Core
----------------

To build Super8 Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Super8 Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/super8/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/super8pay/super8
...
For example:
```bash
URL=https://github.com/crowning-/super8.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit super8=${COMMIT} --url super8=${URL} ../super8/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit super8=${COMMIT} --url super8=${URL} ../super8/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit super8=${COMMIT} --url super8=${URL} ../super8/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the super8 git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../super8/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/super8
SIGPATH=/some/root/path/super8-detached-sigs

./bin/gbuild --url super8=${BTCPATH},signature=${SIGPATH} ../super8/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/super8-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/super8-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/super8-osx-build.assert
```
...
[super8/gitian.sigs](https://github.com/super8pay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
